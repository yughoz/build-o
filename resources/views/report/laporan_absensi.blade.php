    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('#myTable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('list.absensi-datatable') }}",
                    data: function (d) {
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function ( data, type, row ) {
                            let statusMasuk = (data == 1) ? "Masuk" : "Tidak Masuk";
                            let label = (data == 1) ? "info" : "warning";
                            return '<div class="badge badge-'+label+'">'+ statusMasuk +'</div>';
                        }
                    },
                    {
                        data: 'alasan_tidak_masuk', 
                        render: function ( data, type, row ) {
                            let alasanTidakMasuk = (data != "" && data != null) ? data : "-";
                            return alasanTidakMasuk;
                        }
                    },
                    {
                        data: 'waktu_mulai_kerja',
                        render: function ( data, type, row ) {
                            waktuMulaiKerja = (data != null) ? data : "-";
                            return waktuMulaiKerja;
                        }
                    },
                    {
                        data: 'waktu_akhir_kerja',
                        render: function ( data, type, row ) {
                            waktuAkhirKerja = (data != null) ? data : "-";
                            return waktuAkhirKerja;
                        }
                    },
                    {
                        data: 'kegiatan',
                        render: function ( data, type, row ) {
                            kegiatan = (data != null) ? data : "-";
                            if (kegiatan.length > 20) {
                                kegiatan = kegiatan.substring(0, 20) + " ...";
                            }
                            return kegiatan;
                        }
                    },
                    {
                        render: function ( data, type, row ) {
                            koordinator = (row.koordinator != null) ? row.koordinator.pegawai.nama : "-";
                            return koordinator;
                        }
                    },
        ...
        function exportData(type){ 
            ...
            switch (type) {
                case "excel":
                    url = "{{ route('export.excel-absensi') }}"+"?"+searchParams;
                    break;
                case "pdf":
                    url = "{{ route('export.pdf-absensi') }}"+"?"+searchParams;
                    break;
                default:
                    url = "#";
                    break;
